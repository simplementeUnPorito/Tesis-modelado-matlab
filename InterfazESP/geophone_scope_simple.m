...
% geophone_scope_simple — Interfaz ESP32+PSoC5 multi-canal vía USB Serial
%
% Protocolo RX — 6 bytes por paquete (Serial USB desde ESP maestro):
%   Data:     [0x56][node_id][0x00][b2][b1][b0]  int24 big-endian signed
%   HB:       [0x56][node_id][0x01][pga_code][vdac_val][tx_mode]
%   CFG-ADC:  [0x56][node_id][0x02][res][fsH][fsL]
%   CFG-PGA:  [0x56][node_id][0x03][pga_code][vrefH][vrefL]
%   CFG-VREF: [0x56][node_id][0x04][pgavdac_code][vdac_val][0x00]
%   ACK:      [0x56][node_id][0x07][cmd][val][0x00]
%   READY:    [0x56][0xFF][0xFE][n_nodes][0x00][0x00]  (todos armados)
%   node_id:  0x00=maestro/martillo, 0x01-0x0N=esclavos/geófonos
%
% Comandos TX → Maestro ESP: [0xAB][cmd][param][cmd XOR param]
%   [0xAB][0xA1][en][cs]        stream on/off
%   [0xAB][0xA2][n][cs]         arm N nodos esclavos
%   [0xAB][0xA3][0][cs]         start muestreo (dispara sincronización)
%   [0xAB][0xA4][0][cs]         stop muestreo
%   [0xAB][0xA5][0][cs]         request status
...
%
% Conexión: Serial USB al ESP maestro (COM port, 115200 baud)
% Logs: logs\<timestamp>_machine.log  (detallado, para debugging)
%       logs\<timestamp>_human.log   (simplificado, para el usuario)
% Entidades: scope_config.mat  |  Muestras: datos\<entidad>.mat
    ...
    S.isConnected  = false;
    S.logFid       = -1;
    S.logFidH      = -1;   % log legible para humanos
    S.tickCount    = 0;
    S.totalBytes   = 0;
    S.streamTimer  = [];
    S.rxBuf        = uint8([]);
    ...
    S.parseState   = 0;
    S.pktBuf       = zeros(6,1,'uint8');   % 6 bytes: [0x56][node_id][type][b2][b1][b0]
    S.pktIdx       = 0;
    S.pktNodeId    = 0;                    % node_id del paquete en curso

    % Multi-canal: canal 0 = maestro/martillo, 1-N = esclavos/geófonos
    MAX_CHANNELS = 4;
    for chk = 1:MAX_CHANNELS
        S.chanNotch{chk} = zeros(0,1);
        S.chanFilt{chk}  = zeros(0,1);
        S.chanN{chk}     = zeros(0,1);
    end
    S.numChannels   = 1;   % se actualiza con paquete READY
    S.hammerEnabled = false;   % activar cuando haya hardware de martillo
    S.debugESP      = false;   % modo debug: esclavos generan rampa en lugar de PSoC
    ...
    S.pktTotalBytes   = 6;     % 6 bytes: [0x56][node_id][type][b2][b1][b0]
    ...
    % --- Conexion Serial USB ---
    pConn = uipanel(fig,'Title','Conexion Serial USB','Position',[RX 1053 RW 116]);
    ...
    lblCfgInfo = uilabel(pConn,'Text','ADC: —  fs: —  Nodos: —',...
        'Position',[8 8 210 20],'FontSize',8,'FontAngle','italic');

    % --- Sincronizacion multi-nodo ---
    pSync = uipanel(fig,'Title','Sincronización ESP-NOW','Position',[RX 883 RW 162]);
    btnArm  = uibutton(pSync,'Text','ARM','Position',[8 120 65 26],...
        'BackgroundColor',[0.95 0.75 0.2],'FontWeight','bold',...
        'ButtonPushedFcn',@onArmToggle,...
        'Tooltip','Armar todos los nodos esclavos (espera ARM_ACK)');
    btnSyncStart = uibutton(pSync,'Text','SYNC START','Position',[80 120 130 26],...
        'BackgroundColor',CLR_START,'FontWeight','bold',...
        'ButtonPushedFcn',@onSyncStart,...
        'Tooltip','Enviar T_start a todos los nodos — inicia muestreo sincronizado');
    btnSyncStop = uibutton(pSync,'Text','STOP','Position',[8 88 65 26],...
        'BackgroundColor',CLR_STOP,'FontColor',[1 1 1],'FontWeight','bold',...
        'ButtonPushedFcn',@onSyncStop,...
        'Tooltip','Detener muestreo en todos los nodos');
    btnHammer = uibutton(pSync,'Text','Martillo: OFF','Position',[80 88 130 26],...
        'BackgroundColor',CLR_OFF,'FontSize',9,...
        'ButtonPushedFcn',@onHammerToggle,...
        'Tooltip','Activar cuando el acondicionamiento de señal del martillo esté conectado');
    btnDebugESP = uibutton(pSync,'Text','Debug ESP: OFF','Position',[8 58 202 26],...
        'BackgroundColor',CLR_OFF,'FontSize',9,...
        'ButtonPushedFcn',@onDebugToggle,...
        'Tooltip','Esclavos generan rampa numérica — probar pipeline sin PSoC');
    lblSyncStatus = uilabel(pSync,'Text','Estado: sin conexión',...
        'Position',[8 34 210 18],'FontSize',8,'FontAngle','italic');
    lblReadyNodes = uilabel(pSync,'Text','Nodos listos: 0 / ?',...
        'Position',[8 14 210 18],'FontSize',8);

    % --- Stream + Ver crudo + Modo Vista ---
    pCtrl = uipanel(fig,'Title','Stream','Position',[RX 745 RW 136]);
    ...
    % Alias: misma trama [0xAB][cmd][param][XOR] — usada para comandos ESP
    function bytes = espCmd(cmd, param)
        bytes = psocCmd(cmd, param);
    end
    ...
    function pruneOldLogs(dir_, pattern, maxKeep)
        files = dir(fullfile(dir_, pattern));
        if numel(files) >= maxKeep
            [~, ord] = sort([files.datenum]);
            toDelete = files(ord(1:end-maxKeep+1));
            for k = 1:numel(toDelete)
                try delete(fullfile(dir_, toDelete(k).name)); catch, end
            end
        end
    end
    ...
    function logMsg(msg)
        ...
        if S.logFidH>0, fprintf(S.logFidH,'%s\n',char(line)); end
    end
    function logMsgH(msg)
        % Solo al log humano (mensajes de resumen, no verbosos)
        if S.logFidH>0
            fprintf(S.logFidH,'[%s] %s\n',datestr(now,'HH:MM:SS'),char(msg));
        end
    end
    ...
    function appendSamples(newNotch, nodeIds)
        nNew = numel(newNotch);
        ...
        % Distribuir por canal (node_id+1 como índice 1-based)
        if nargin >= 2 && ~isempty(nodeIds)
            for ci = 1:nNew
                ch = double(nodeIds(ci)) + 1;  % node 0 → ch 1, node 1 → ch 2, etc.
                if ch >= 1 && ch <= 4
                    S.chanNotch{ch}(end+1,1) = newNotch(ci);
                    S.chanN{ch}(end+1,1)     = nIdx(ci);
                    if numel(S.chanN{ch}) > S.maxPoints
                        S.chanNotch{ch} = S.chanNotch{ch}(end-S.maxPoints+1:end);
                        S.chanN{ch}     = S.chanN{ch}(end-S.maxPoints+1:end);
                    end
                end
            end
        end
    ...
    function onConnectToggle(~,~)
        if ~S.isConnected
            ...
            try
                ...
                S.pktBuf=zeros(6,1,'uint8'); S.pktIdx=0; S.pktNodeId=0;
                ...
                pruneOldLogs(logsDir, '*_machine.log', 10);
                pruneOldLogs(logsDir, '*_human.log',   10);
                lname  = fullfile(logsDir, sprintf('scope_%s_machine.log', ts));
                lnameH = fullfile(logsDir, sprintf('scope_%s_human.log',   ts));
                S.logFid  = fopen(lname,  'w');
                S.logFidH = fopen(lnameH, 'w');
                if S.logFid>0
                    fprintf(S.logFid,'=== geophone_scope_simple ESP — %s ===\n',datestr(now));
                    fprintf(S.logFid,'Serial: %s @ %d baud\n', com, baud);
                    fprintf(S.logFid,'[Log detallado — para debugging]\n\n');
                end
                if S.logFidH>0
                    fprintf(S.logFidH,'=== Sesión geofónica — %s ===\n', datestr(now));
                    fprintf(S.logFidH,'Conexión: ESP maestro %s @ %d baud\n\n', com, baud);
                end
                lblStat.Text = 'CONECTADO: ' + com;
                logMsg("Conectado " + com + " @ " + string(baud));
                logMsg("Log máquina: " + string(lname));
                logMsg("Log humano: "  + string(lnameH));
                lblSyncStatus.Text = 'Estado: conectado, sin armar';
                requestAndParseConfig();
                saveConfig(); startStreamTimer();
            catch e
                S.sp=[]; S.isConnected=false;
                logMsg("Error Serial: "+string(e.message));
            end
        else
            stopStreamTimer(); S.streamEnabled=false;
            try
                if ~isempty(S.sp)
                    write(S.sp, espCmd(0xA1, 0), 'uint8');
                    pause(0.02);
                end
            catch, end
            ...
            if S.logFidH>0
                fprintf(S.logFidH,'\nSesión terminada: %s\n', datestr(now));
                try fclose(S.logFidH); catch,end; S.logFidH=-1;
            end
            S.sp=[]; S.isConnected=false; lblStat.Text='DESCONECTADO';
            S.configReceived=false;
            lblCfgInfo.Text='ADC: —  fs: —  Nodos: —';
            lblSyncStatus.Text = 'Estado: sin conexión';
            saveConfig();
        end
        updateBtnStates();
    end
    ...
    % =====================================================================
    % Sincronización multi-nodo
    % =====================================================================
    function onArmToggle(~,~)
        if ~S.isConnected, logMsg("No conectado."); return; end
        try
            write(S.sp, espCmd(0xA2, uint8(S.numChannels)), 'uint8');
            lblSyncStatus.Text = 'Estado: ARM enviado...';
            logMsg(sprintf('ARM → maestro (%d nodos)', S.numChannels));
            logMsgH(sprintf('Armando %d nodo(s) esclavos...', S.numChannels));
        catch ex
            logMsg('ARM error: '+string(ex.message));
        end
    end
    ...
    function onSyncStart(~,~)
        if ~S.isConnected, logMsg("No conectado."); return; end
        try
            write(S.sp, espCmd(0xA3, 0), 'uint8');
            lblSyncStatus.Text = 'Estado: START enviado — muestreo en curso';
            logMsg('SYNC START → maestro');
            logMsgH(sprintf('=== MUESTREO INICIADO — %s ===', datestr(now,'HH:MM:SS')));
        catch ex
            logMsg('START error: '+string(ex.message));
        end
    end
    ...
    function onSyncStop(~,~)
        if ~S.isConnected, logMsg("No conectado."); return; end
        try
            write(S.sp, espCmd(0xA4, 0), 'uint8');
            lblSyncStatus.Text = 'Estado: STOP enviado';
            logMsg('SYNC STOP → maestro');
            logMsgH(sprintf('=== MUESTREO DETENIDO — %s ===', datestr(now,'HH:MM:SS')));
        catch ex
            logMsg('STOP error: '+string(ex.message));
        end
    end

    function onHammerToggle(~,~)
        S.hammerEnabled = ~S.hammerEnabled;
        if S.hammerEnabled
            btnHammer.Text = 'Martillo: ON';
            btnHammer.BackgroundColor = CLR_ON;
            logMsg('Martillo ACTIVADO — verificar acondicionamiento de señal antes de conectar');
            logMsgH('Martillo activado (canal 0 activo)');
        else
            btnHammer.Text = 'Martillo: OFF';
            btnHammer.BackgroundColor = CLR_OFF;
            logMsg('Martillo DESACTIVADO (stub)');
        end
    end

    function onDebugToggle(~,~)
        if ~S.isConnected, logMsg('No conectado.'); return; end
        S.debugESP = ~S.debugESP;
        try
            write(S.sp, espCmd(0xA7, uint8(S.debugESP)), 'uint8');
            if S.debugESP
                btnDebugESP.Text = 'Debug ESP: ON';
                btnDebugESP.BackgroundColor = [0.30 0.80 1.00];
                logMsg('Debug ESP ON — esclavos generan rampa (sin PSoC)');
                logMsgH('Modo debug activado: rampa numérica en canales esclavos');
            else
                btnDebugESP.Text = 'Debug ESP: OFF';
                btnDebugESP.BackgroundColor = CLR_OFF;
                logMsg('Debug ESP OFF — esclavos leen PSoC via SPI');
            end
        catch ex
            S.debugESP = ~S.debugESP;   % revertir si falla
            logMsg('Debug toggle error: ' + string(ex.message));
        end
    end
    function onClear(~,~)
    ...
    function onStreamTickImpl()
        ...
        if nAvail<=0
            if S.streamEnabled && S.totalBytes==0 && ~S.noDataWarned && ...
                ...
                logMsg('SIN RX: TCP conectado pero 0 bytes. Verificar ESP maestro y que el stream esté activo.');
                if S.logFid>0
        ...
        newNotch=zeros(0,1); newNodeId=zeros(0,1,'uint8');
        i=1; n=numel(S.rxBuf);
        VALID_TYPES = uint8([0, 1, 2, 3, 4, 7, hex2dec('FE')]);
        while i<=n
            b=S.rxBuf(i);
            switch S.parseState
                case 0  % buscar sync 0x56
                    ...
                    i=i+1;
                case 1  % byte 2 = node_id (cualquier valor 0x00-0xFF)
                    S.pktNodeId = double(b);
                    S.pktIdx=2; S.pktBuf(2)=b; S.parseState=2; i=i+1;
                case 2  % byte 3 = tipo — validar
                    if any(b==VALID_TYPES)
                        S.pktIdx=3; S.pktBuf(3)=b; S.parseState=3; i=i+1;
                    else
                        S.rxBadTypes = S.rxBadTypes + 1;
                        S.parseState=0;
                        if b~=uint8(0x56), i=i+1; end
                    end
                case 3  % acumular bytes restantes (pktTotalBytes - 3 bytes de datos)
                    S.pktIdx=S.pktIdx+1; S.pktBuf(S.pktIdx)=b; i=i+1;
                    if S.pktIdx==S.pktTotalBytes
                        pkt=double(S.pktBuf(1:S.pktTotalBytes));
                        % pkt(1)=0x56, pkt(2)=node_id, pkt(3)=type, pkt(4:6)=datos
                        nid  = pkt(2);
                        ptype= pkt(3);
                        S.rxFrames = S.rxFrames + 1;
                        switch ptype
                            case 0  % data — int24 big-endian signed
                                u = pkt(4)*65536 + pkt(5)*256 + pkt(6);
                                v = u - (pkt(4)>=128)*16777216;
                                newNotch(end+1,1)=v;     %#ok<AGROW>
                                newNodeId(end+1,1)=uint8(nid); %#ok<AGROW>
                            case 1  % heartbeat
                                hb_pga  = pkt(4);
                                hb_vdac = pkt(5);
                                hb_mode = pkt(6);
                                if hb_pga <= 8 && hb_mode <= 1
                                    ...
                                    if nid == 0
                                        % heartbeat del maestro = estado global
                                        lblSyncStatus.Text = sprintf('Estado maestro: %d', hb_mode);
                                    end
                                    logMsg(sprintf('HB[%d] bytes=%d PGA:%s VDAC:%d modo:%s',...
                                        nid, S.totalBytes, pgaGainStr(hb_pga), hb_vdac, modeStr));
                                    checkPending('PGA',  hb_pga);
                                    checkPending('VDAC', hb_vdac);
                                else
                                    if S.logFid>0
                                        fprintf(S.logFid,'  [desync-HB nid=%d] pga=0x%02X\n',nid,hb_pga);
                                    end
                                end
                            case 3  % cfg PGA
                                newCode = pkt(4);
                                if newCode <= 8
                                    ...
                                    logMsg(sprintf('Confirmó PGA[%d]: %s', nid, pgaGainStr(newCode)));
                                    checkPending('PGA', newCode);
                                end
                            case 4  % cfg VRef
                                newCode = pkt(4); newVdac = pkt(5);
                                if newCode <= 8
                                    ...
                                    updateVdacCalcDisplay();
                                    logMsg(sprintf('Confirmó VRef[%d]: pgavdac=%s VDAC=0x%02X',...
                                        nid, pgaGainStr(newCode), newVdac));
                                    checkPending('PGAVDAC', newCode);
                                    checkPending('VDAC', newVdac);
                                end
                            case 7  % ACK
                                ack_cmd = pkt(4); ack_val = pkt(5);
                                switch ack_cmd
                                    case hex2dec('A1')
                                        streamVal = double(ack_val ~= 0);
                                        S.streamEnabled = logical(streamVal);
                                        logMsg(sprintf('ACK STREAM=%d [nid=%d]', streamVal, nid));
                                        updateBtnStates();
                                    case hex2dec('A2')
                                        logMsg(sprintf('ACK ARM enviado [nid=%d]', nid));
                                        lblSyncStatus.Text = 'Estado: ARM enviado, esperando nodos';
                                    case hex2dec('A3')
                                        logMsg(sprintf('ACK START [nid=%d]', nid));
                                        lblSyncStatus.Text = 'Estado: MUESTREO en curso';
                                        logMsgH('=== INICIO DE MUESTREO SINCRONIZADO ===');
                                    case hex2dec('A4')
                                        logMsg(sprintf('ACK STOP [nid=%d]', nid));
                                        lblSyncStatus.Text = 'Estado: muestreo detenido';
                                        logMsgH('=== FIN DE MUESTREO ===');
                                    case hex2dec('A8')
                                        modeVal = double(ack_val ~= 0);
                                        S.psoc_tx_mode = modeVal;
                                        btnTxMode.Text = sprintf('PSoC: %s', iif(modeVal==0,'Crudo','Filter'));
                                        logMsg(sprintf('ACK TXMODE=%d', modeVal));
                                        checkPending('TXMODE', modeVal);
                                    case hex2dec('AA')
                                        S.servo_vdac = double(ack_val);
                                        updateVdacCalcDisplay();
                                        logMsg(sprintf('ACK VDAC=0x%02X', ack_val));
                                    ...
                                    case hex2dec('A9')
                                        if ack_val <= 8
                                            ...
                                            updateVdacCalcDisplay();
                                            logMsg(sprintf('ACK PGAvdac=%s', pgaGainStr(ack_val)));
                                            checkPending('PGAVDAC', ack_val);
                                        end
                                    otherwise
                                        logMsg(sprintf('ACK cmd=0x%02X val=%d [nid=%d]',ack_cmd,ack_val,nid));
                                end
                            case hex2dec('FE')  % READY: todos los nodos armados
                                nReady = pkt(4);
                                S.numChannels = max(1, nReady);
                                lblReadyNodes.Text = sprintf('Nodos listos: %d', nReady);
                                lblSyncStatus.Text = sprintf('Estado: %d nodo(s) armados — listo para START', nReady);
                                logMsg(sprintf('READY: %d nodos armados', nReady));
                                logMsgH(sprintf('Nodos armados: %d — presionar SYNC START para iniciar', nReady));
                        end
                        S.parseState=0;
                    end
                otherwise
        ...
        appendSamples(newNotch, newNodeId);
        replot(); updateInfo();
        drawnow limitrate;
    end
    ...
    function onClose(~,~)
        ...
        if S.isConnected&&~isempty(S.sp)
            try write(S.sp, espCmd(0xA1, 0), 'uint8'); pause(0.02); catch,end
